...
pub struct F32Keywords {
    ...
    /// Held in f64 because ZZERO can have magnitudes (e.g. ~2.7e8 in DES `.fz`)
    /// where f32 storage truncates below the noise floor.
    scale: f64,
...
impl F64Keywords {
    ...
    /// f64 arithmetic, see F32Keywords::unquantize for rationale.
    fn unquantize(&mut self, value: i32) -> f32 {
        ...
        match &mut self.quantiz {
            Quantiz::NoDither => ((value as f64) * self.scale + self.zero) as f32,
            Quantiz::SubtractiveDither1 { i1 } => {
                let ri = RAND_VALUES[*i1] as f64;
                *i1 = (*i1 + 1) % N_RANDOM;
                (((value as f64) - ri + 0.5) * self.scale + self.zero) as f32
            }
...
impl F32Keywords {
    /// Unquantize the integer decoded value to the real floating point value.
    /// Computed in f64 to avoid precision loss when |ZZERO| is large
    /// (e.g. DES `.fz` files store ZZERO ~ 2.7e8, where f32 step is ±32).
    fn unquantize(&mut self, value: i32) -> f32 {
        if let Some(z_blank) = self.z_blank {
            if z_blank == value {
                return f32::NAN;
